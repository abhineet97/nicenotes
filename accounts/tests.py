...
class AccountsTest(APITestCase):
    def setUp(self):
        self.test_user = User.objects.create_user(
            "1234567890", "thisisapassword", full_name="Test_User"
        )
    ...
    def test_list_users(self):
        User.objects.create_user(
            "0987654321", "passingthepass", full_name="Dope Tester"
        )
        User.objects.create_user(
            "1234509876", "passthesauceplease", full_name="Happy Tester"
        )
    ...
    def test_user_login(self):
        data = {"username": "1234567890", "password": "thisisapassword"}
    ...
    def test_notes_shared_with_current_user(self):
        ...
        serialized = UserSerializer(
            User.objects.get(auth_token=self.test_user_token), show_access_only=True
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
